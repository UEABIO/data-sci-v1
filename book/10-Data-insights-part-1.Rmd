...
With the information about the distribution, and positions of the mean and median, which of these two measures of central tendency is most appropriate for this data? `r mcq(c(answer = "mean", "median"))`

```{solution}
we can clearly see that the mean value has been significantly "right-shifted" by the long tail of the data distribution. However,this is much less apparent for the median. In this way we can say that the median is less sensitive to the distribution of the data than the mean is.
```
...
```{task}
Produce a plot which allows you to look at the distribution of penguin body mass observations by species
```
